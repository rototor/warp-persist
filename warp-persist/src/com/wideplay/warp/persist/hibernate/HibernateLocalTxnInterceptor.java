...
class HibernateLocalTxnInterceptor implements MethodInterceptor {
	...
	HibernateLocalTxnInterceptor(Provider<Session> sessionProvider) {
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable {
		...
		//read-only txn?
		final boolean isReadOnly = TransactionType.READ_ONLY.equals(transactional.type());


		FlushMode savedFlushMode = null;
		try {
			...
			try {
				...
				// We can not rollback the transaction, because other stuff will fail here very subtil...
				// if (isReadOnly)
				//	txn.rollback();
				// else
					txn.commit();
			...
			if (session.isOpen() && isReadOnly && savedFlushMode != null)
				session.setHibernateFlushMode(savedFlushMode);
		}
	}
